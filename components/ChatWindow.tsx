...
export const ChatWindow: React.FC<ChatWindowProps> = ({ userData }) => {
  ...
  return (
    <div className="flex flex-col h-[70vh] md:h-[750px] bg-white dark:bg-[#000814] rounded-[30px] md:rounded-[50px] shadow-2xl overflow-hidden border border-gray-100 dark:border-blue-900/40">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 md:space-y-12 bg-white dark:bg-[#000814] scroll-smooth">
        {messages.map((msg, i) => (
            ...
            <div className={`max-w-[95%] md:max-w-[92%] relative ${msg.role === 'user' ? 'text-right' : 'text-left w-full'}`}>
              ...
              {msg.role === 'model' ? (
                <div className="bg-[#f8fafb] dark:bg-[#000e2e] p-6 md:p-10 rounded-[30px] md:rounded-[40px] border border-gray-50 dark:border-blue-900/40 shadow-sm relative group">
                  ...
                  <div className="whitespace-pre-wrap text-[13px] md:text-[15px] leading-loose tracking-tight font-medium text-slate-700 dark:text-blue-100/80">
                    {msg.content}
                  </div>
                  
                  {msg.groundingUrls && (
                    <div className="mt-8 md:mt-10 pt-6 md:pt-10 border-t border-gray-100 dark:border-blue-900/30">
                      <p className="text-[7px] md:text-[8px] font-black uppercase text-emerald-600 tracking-[0.3em] mb-4">
                         <span className="text-emerald-500 font-normal">{brandPhrase}</span> Sources
                      </p>
                      <div className="flex flex-wrap gap-2 md:gap-3">
                        {msg.groundingUrls.map((g, j) => (
                          <a key={j} href={g.web.uri} target="_blank" rel="noreferrer" className="text-[9px] md:text-[10px] bg-white dark:bg-blue-900/20 px-3 md:px-4 py-1.5 md:py-2 rounded-full text-blue-800 dark:text-blue-200 hover:bg-emerald-500 hover:text-white transition-all border border-gray-100 dark:border-blue-900/50 shadow-sm">{g.web.title}</a>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="mt-6 md:mt-10 flex justify-between items-center">
                    <span className="text-[7px] md:text-[9px] font-black uppercase tracking-widest opacity-20">{msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    <button 
                      onClick={() => handleSpeak(msg.content)} 
                      className={`p-2 rounded-lg transition-all ${isSpeaking ? 'text-emerald-500 bg-emerald-500/10 animate-pulse' : 'text-gray-300 hover:text-emerald-500'}`}
                    >
                ...
                <div className="bg-[#001040] text-white p-4 md:p-6 rounded-[20px] md:rounded-[30px] rounded-tr-none shadow-lg inline-block max-w-[90%] text-left">
                  {msg.image && (
                    <div className="mb-3 md:mb-4 rounded-xl overflow-hidden">
                      <img src={msg.image} alt="Attachment" className="w-full h-auto max-h-48 md:max-h-64 object-contain" />
                    </div>
                  )}
                  <div className="whitespace-pre-wrap text-[13px] md:text-[15px] font-bold leading-relaxed">{msg.content}</div>
                  <div className="mt-2 text-[7px] md:text-[8px] font-black uppercase opacity-40 text-right tracking-widest">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-[#f8fafb] dark:bg-blue-950/40 p-6 rounded-[25px] animate-pulse flex flex-col space-y-4 w-full md:w-4/5">
              <div className="flex items-center space-x-2">
                <div className="w-2 h-2 bg-emerald-500 rounded-full animate-bounce"></div>
                <span className="text-[8px] font-black uppercase text-emerald-600 tracking-widest">
                   Processing <span className="text-emerald-500 font-normal">{brandPhrase}</span>...
                </span>
              </div>
              <div className="h-2 bg-gray-200 dark:bg-blue-900/40 rounded-full w-4/5"></div>
              <div className="h-2 bg-gray-200 dark:bg-blue-900/40 rounded-full w-2/3"></div>
            </div>
          </div>
        )}
      </div>

      <div className="p-4 md:p-8 bg-[#fcfdfe] dark:bg-blue-950/40 border-t border-gray-100 dark:border-blue-900/40">
          ...
          <div className="flex space-x-2 md:space-x-4 mb-4 md:mb-8 overflow-x-auto pb-2 scrollbar-hide">
            {discussionThemes.map((theme, i) => (
              <button 
                ...
                className="flex items-center space-x-2 px-4 md:px-6 py-2 md:py-3 bg-white dark:bg-[#001040] border border-gray-100 dark:border-blue-900 rounded-full shadow-sm hover:border-emerald-500 transition-all whitespace-nowrap"
              >
                <span className="text-sm md:text-lg">{theme.icon}</span>
                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-[#001040] dark:text-blue-100">{theme.title}</span>
              </button>
            ))}
          </div>
        )}
        ...
        {selectedFile && (
          <div className="mb-4 bg-white dark:bg-blue-900 p-4 rounded-2xl flex items-center justify-between border border-emerald-500/30">
            <div className="flex items-center space-x-3">
              <span className="text-xl">{selectedFile.type.startsWith('image/') ? '🖼️' : '📄'}</span>
              <p className="text-[9px] font-bold text-gray-500 truncate max-w-[150px]">{selectedFile.name}</p>
            </div>
            <button onClick={() => setSelectedFile(null)} className="text-red-500 font-bold p-1">×</button>
          </div>
        )}
        ...
        <div className="flex items-center space-x-2 md:space-x-4 bg-white dark:bg-[#000814] p-2 md:p-3 rounded-[20px] md:rounded-[35px] border border-gray-100 dark:border-blue-900/50 shadow-sm">
          ...
          <input 
            ...
            className="flex-1 bg-transparent border-none focus:ring-0 text-[13px] md:text-[15px] font-medium dark:text-white placeholder:text-gray-300 dark:placeholder:text-gray-600 outline-none"
          />
